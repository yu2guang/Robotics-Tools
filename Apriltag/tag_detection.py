...
# camera
sys.path.append(os.path.abspath(os.path.join('..', 'Realsense_depth_camera')))
from Realsense_depth_camera.realsense_cam import realsense_cam
...
def detect_tags(img, cam_at, tag_size, tag_list=[]):
    origin_img = img.copy()
...
if __name__ == '__main__':
    # initialize camera & tag board
    cam = realsense_cam('635202000911')
    tagboard_dict, tag_size = tag_boards('board4x6')

    # detect tags
    while True:
        color_img, _ = cam.get_image()
        detect_img, tag_IDs, tag_img_pts = detect_tags(color_img, cam.intrinsic_at, tag_size)
        cv2.imshow('Detected April Tag', detect_img)
